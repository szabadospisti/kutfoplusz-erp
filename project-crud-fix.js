...
(function installProjectCrud(){
  ...
  const customerName=p=>{const id=p.customerId??p.customer_id;const c=customers().find(x=>String(x.id)===String(id));return c?.company_name||c?.name||p.customer_name||'—'};
  async function persist(){
    if(typeof window.save!=='function')throw new Error('A központi mentés nem érhető el.');
    const result=await window.save();
    if(result!==true)throw new Error('A Supabase mentés nem sikerült.');
    return true;
  }
  ...
  function openProjectEditor(id){
    ...
    const customerId=p.customerId??p.customer_id??'';document.getElementById('kpProjectEdit')?.remove();
    ...
    <div class="field"><label>Projekt neve</label><input required class="input" id="kpPName" value="${esc(p.name||'')}"></div><div class="field"><label>Projektazonosító</label><input class="input" value="${esc(p.project_number||'')}" readonly></div>
    ...
    <div class="field"><label>Szerződéses érték</label><input class="input" type="number" step="0.01" id="kpPValue" value="${Number(p.contract_value)||0}"></div><div class="field"><label>Tervezett kezdés</label><input class="input" type="date" id="kpPStart" value="${esc(p.start_date||'')}"></div><div class="field"><label>Tervezett befejezés</label><input class="input" type="date" id="kpPEnd" value="${esc(p.planned_end_date||'')}"></div>
    ...
    document.body.appendChild(m);m.querySelector('[data-close]').onclick=closeEditor;m.querySelector('[data-cancel]').onclick=closeEditor;m.querySelector('[data-delete]').onclick=()=>window.kpDeleteProject(id);
    m.querySelector('[data-save]').onclick=async()=>{
      const btn=m.querySelector('[data-save]');btn.disabled=true;const before=JSON.parse(JSON.stringify(p));
      p.name=m.querySelector('#kpPName').value.trim();p.location=m.querySelector('#kpPLocation').value.trim();p.customerId=m.querySelector('#kpPCustomer').value||'';p.customer_id=p.customerId;p.status=m.querySelector('#kpPStatus').value||p.status||'interest';p.contract_value=Number(m.querySelector('#kpPValue').value)||0;p.start_date=m.querySelector('#kpPStart').value||'';p.planned_end_date=m.querySelector('#kpPEnd').value||'';
      try{await persist();closeEditor();window.render();if(typeof window.toast==='function')window.toast('Projekt módosítva és Supabase-ben mentve');}
      catch(e){Object.assign(p,before);btn.disabled=false;console.error(e);alert('A projekt módosítása nem lett véglegesítve.\n\n'+e.message);}
    };
  }
  window.kpEditProject=openProjectEditor;
  window.kpDeleteProject=async function(id){const p=projects().find(x=>String(x.id)===String(id));if(!p)return;if(!confirm(`Biztosan törlöd a(z) „${p.name||p.project_number||id}” projektet?\n\nA törlés végleges.`))return;const old=window.db.projects.slice();window.db.projects=window.db.projects.filter(x=>String(x.id)!==String(id));try{await persist();closeEditor();window.render();if(typeof window.toast==='function')window.toast('Projekt törölve és Supabase-ben mentve');}catch(e){window.db.projects=old;alert('A projekt törlése nem sikerült.\n\n'+e.message);}};
  ...
  const oldRender=window.render;window.render=function(){oldRender();if(window.current==='projects'){const root=document.querySelector('.content');if(!root)return;root.querySelectorAll('[data-edit]').forEach(b=>b.onclick=()=>openProjectEditor(b.dataset.edit));root.querySelectorAll('[data-delete]').forEach(b=>b.onclick=()=>window.kpDeleteProject(b.dataset.delete));const n=root.querySelector('#kpNewProject');if(n)n.onclick=()=>{if(typeof window.newProject==='function')window.newProject();};}};
