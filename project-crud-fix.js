...
(function installProjectCrud(){
  ...
  async function persist(){if(typeof window.save!=='function')throw new Error('A központi mentés nem érhető el.');if(await window.save()!==true)throw new Error('A Supabase mentés nem sikerült.');return true;}
  ...
  async function openProjectEditor(id){
    ...
    const customerId=p.customerId??p.customer_id??'';const m=document.createElement('div');m.id='kpProjectEdit';m.className='modal';
    ...
    <div class="field"><label>Projekt neve</label><input required class="input" id="kpPName" value="${esc(p.name||'')}"></div><div class="field"><label>Projektazonosító</label><input class="input" value="${esc(p.project_number||p.id||'')}" readonly></div>
  ...
  window.kpDeleteProject=async function(id){const p=projects().find(x=>String(x.id)===String(id));if(!p)return;if(!confirm(`Biztosan törlöd a(z) „${p.name||p.project_number||id}” projektet?\n\nA törlés végleges.`))return;const old=window.db.projects.slice();window.db.projects=window.db.projects.filter(x=>String(x.id)!==String(id));try{await persist();closeEditor();if(page()==='projects')window.render();if(typeof window.toast==='function')window.toast('Projekt törölve és Supabase-ben mentve');}catch(e){window.db.projects=old;alert('A projekt törlése nem sikerült.\n\n'+e.message);}};
