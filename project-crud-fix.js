 ...
 * Ugyanaz a mentési modell, mint a működő Gépparknál.
 */
(function installProjectCrud(){
  ...
  function persist(){
    try{
      ...
      if(result&&typeof result.catch==='function')result.catch(e=>console.error('Projekt Supabase háttérszinkron:',e));
      return true;
    }catch(e){console.error('Projekt mentés:',e);return false;}
  }
  ...
  function openProjectEditor(id){
    ...
    m.querySelector('[data-close]').onclick=closeEditor;m.querySelector('[data-cancel]').onclick=closeEditor;
    m.querySelector('[data-delete]').onclick=()=>window.kpDeleteProject(id);
    m.querySelector('[data-save]').onclick=()=>{
      const before=JSON.parse(JSON.stringify(p));
      p.name=m.querySelector('#kpPName').value.trim();p.location=m.querySelector('#kpPLocation').value.trim();
      p.customerId=m.querySelector('#kpPCustomer').value||'';p.customer_id=p.customerId;p.status=m.querySelector('#kpPStatus').value||p.status||'interest';
      p.contract_value=Number(m.querySelector('#kpPValue').value)||0;p.start_date=m.querySelector('#kpPStart').value||'';p.planned_end_date=m.querySelector('#kpPEnd').value||'';
      const ok=persist();if(!ok){Object.assign(p,before);alert('A projekt módosítása nem sikerült.');return;}
      closeEditor();window.render();if(typeof window.toast==='function')window.toast('Projekt módosítva');
    };
  }
  window.kpEditProject=openProjectEditor;
  window.kpDeleteProject=function(id){const p=projects().find(x=>String(x.id)===String(id));if(!p)return;if(!confirm(`Biztosan törlöd a(z) „${p.name||p.project_number||id}” projektet?\n\nA törlés végleges.`))return;const old=window.db.projects.slice();window.db.projects=window.db.projects.filter(x=>String(x.id)!==String(id));const ok=persist();if(!ok){window.db.projects=old;return;}closeEditor();window.render();if(typeof window.toast==='function')window.toast('Projekt törölve');};
