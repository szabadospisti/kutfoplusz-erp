...
(function(){
  ...
  async function deleteProject(id){
    ...
    if(p.supabaseId)await rest('projects?id=eq.'+encodeURIComponent(p.supabaseId),{method:'DELETE',headers:{Prefer:'return=minimal'}});
    d.projects=d.projects.filter(x=>String(x.id)!==String(id));
    if(typeof save==='function')save();if(typeof closeDrawer==='function')closeDrawer();if(typeof closeModal==='function')closeModal();if(typeof nav==='function')nav('projects');if(typeof toast==='function')toast('Projekt törölve');return true;
  }
  async function deleteQuote(id){
    const d=data();if(!d)return;
    const q=(d.quotes||[]).find(x=>String(x.id)===String(id));if(!q)return;
    ...
    d.quotes=d.quotes.filter(x=>String(x.id)!==String(id));
    if(typeof save==='function')save();if(typeof render==='function')render();if(typeof toast==='function')toast('Ajánlat törölve');return true;
  }
  async function deleteWorklog(id){
    const d=data();if(!d)return;
    const w=(d.worklogs||[]).find(x=>String(x.id)===String(id));if(!w)return;
    ...
    d.worklogs=d.worklogs.filter(x=>String(x.id)!==String(id));
    if(typeof save==='function')save();if(typeof closeModal==='function')closeModal();if(typeof render==='function')render();if(typeof toast==='function')toast('Munkanapló törölve');return true;
  }
  ...
  function addButton(parent,cls,text,fn){if(!parent||parent.querySelector('.'+cls))return;const b=document.createElement('button');b.type='button';b.className='btn danger small '+cls;b.textContent='🗑️ '+text;b.onclick=fn;parent.appendChild(b)}
  function decorate(){
    const d=data();if(!d)return;
