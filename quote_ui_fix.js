 ...
 *
 * IMPORTANT: keep quoteItems as the canonical data model. DOCX export must
 * never rewrite qty/unit/desc in the live editor state.
 */
(function(){
  "use strict";
  ...
  function disablePreviewFunctions(){if(typeof window.previewQuote==="function"&&!window.__KUTFOPLUSZ_PREVIEW_DISABLED){window.previewQuote=function(){if(typeof window.toast==="function")window.toast("A PDF/előnézet funkció ki van kapcsolva.");return false;};window.__KUTFOPLUSZ_PREVIEW_DISABLED=true;}if(typeof window.printExactPdfPreview==="function"&&!window.__KUTFOPLUSZ_PRINT_PREVIEW_DISABLED){window.printExactPdfPreview=function(){if(typeof window.toast==="function")window.toast("A PDF/nyomtatás funkció ki van kapcsolva.");return false;};window.__KUTFOPLUSZ_PRINT_PREVIEW_DISABLED=true;}if(typeof window.printQuote==="function"&&!window.__KUTFOPLUSZ_PRINT_QUOTE_DISABLED){window.printQuote=function(){if(typeof window.toast==="function")window.toast("A PDF/nyomtatás funkció ki van kapcsolva. DOCX export használható.");return false;};window.__KUTFOPLUSZ_PRINT_QUOTE_DISABLED=true;}}
  ...
  /* PROJECT UI CONSISTENCY FIX
     One project page = one KPI contract and one diameter notation, regardless
  ...
  function normalizeProjectTitle(){
    ...
    if(!/\d+(?:[.,]\d+)?(?:\s*\/\s*\d+(?:[.,]\d+)?)?\s*Ø?\s*mm\b/i.test(txt))return;
    const normalized=normalizeDiameterText(txt);
    if(normalized&&normalized!==txt)h.textContent=normalized;
  }
  function normalizeProjectKpis(){
    const p=currentProject();
    const box=document.querySelector(".project-hero-kpis");
    if(!p||!box)return;
    const cards=Array.from(box.querySelectorAll(":scope > .card"));
    ...
    const values=[
      (Number(p.progress)||0)+"%",
      typeof window.money==="function"?window.money(contract):contract.toLocaleString("hu-HU")+" Ft",
      typeof window.money==="function"?window.money(cost):cost.toLocaleString("hu-HU")+" Ft",
      typeof window.money==="function"?window.money(profit):profit.toLocaleString("hu-HU")+" Ft"
    ];
  ...
  function normalizeProjectUi(){
    if(document.querySelector(".project-hero-kpis")){normalizeProjectKpis();normalizeProjectTitle();}
  }
  function installProjectUiFix(){
    ...
    window.__KUTFOPLUSZ_PROJECT_UI_OBSERVER=observer;
  }

  function patchFunctions(){installDocxDependencyGuard();installManualPriceTracking();disablePreviewFunctions();installProjectUiFix();
  ...
  window.KUTFOPLUSZ_QUOTE_VISUAL_FIX=PATCH;install();window.addEventListener("load",install,{once:false});if(!window.__KUTFOPLUSZ_PREVIEW_REMOVAL_OBSERVER){window.__KUTFOPLUSZ_PREVIEW_REMOVAL_OBSERVER=true;new MutationObserver(function(){removePreviewControls();disablePreviewFunctions();normalizeProjectUi();}).observe(document.documentElement,{childList:true,subtree:true});}
})();
