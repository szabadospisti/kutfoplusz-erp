 ...
 * A géppark natív adatlap modulját nem írjuk felül.
 */
(function installFleetPersistent(){
  if(window.__machineProfileNativeInstalled)return;
  ...
  window.kpSaveMachineEdit=function(e,id){e.preventDefault();const m=db.machines.find(x=>x.id===id);if(!m)return;const o=Object.fromEntries(new FormData(e.target).entries());Object.assign(m,{name:o.name.trim(),model:o.model.trim(),hours:Number(o.hours)||0,service:Number(o.service)||0,status:o.status,notes:o.notes||''});save();closeModal();render();toast('Gép adatai elmentve');};
  window.kpDeleteMachine=function(id){const m=db.machines.find(x=>x.id===id);if(!m)return;if(!confirm(`Biztosan törlöd a(z) „${m.name}” gépet?\n\nA törlés végleges.`))return;db.machines=db.machines.filter(x=>x.id!==id);save();closeModal();render();toast('Gép törölve');};
  window.kpMachineProfile=function(id){if(typeof window.machineProfilePage==='function')return window.machineProfilePage(id);const m=db.machines.find(x=>x.id===id);if(!m)return;openModal('Gép adatlap',`<div><div class="notice">${esc(m.notes||'Nincs megjegyzés.')}</div></div>`);};
