/* Kútfő Plusz ERP – Projekt -> Munkanapló automatikus kapcsolat, stabil V2 */
(function(){
  'use strict';
  if(window.__KP_PROJECT_WORKLOG_AUTO_LINK_V2__) return;
  window.__KP_PROJECT_WORKLOG_AUTO_LINK_V2__=true;
  ...
  function apply(){
    ...
    if(!match&&p){
      var o=document.createElement('option');o.value=s(p.id);o.textContent=s(p.id)+' – '+s(p.name);if(p.supabaseId)o.setAttribute('data-supabase-id',s(p.supabaseId));sel.appendChild(o);match=o;
    }
  ...
  function hook(){
    if(typeof window.newWorklogFor==='function'&&!window.newWorklogFor.__kpAutoV2){
