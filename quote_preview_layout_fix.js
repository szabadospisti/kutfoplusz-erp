...
(function(){
  ...
  function appLogoSrc(){
    ...
    const hit=imgs.find(img=>/kutf|kút|plusz|logo/.test((String(img.alt||"")+" "+String(img.src||"")).toLowerCase()));
    return hit?hit.src:"";
  }
  ...
  function injectFullscreenStyle(){
    ...
    s.textContent=`
      ...
      body:has(.qv-page) .modalbody{flex:1 1 auto!important;min-height:0!important;height:auto!important;max-height:none!important;overflow:auto!important;-webkit-overflow-scrolling:touch!important;padding:12px 24px!important;background:#f3f4f6!important}
      body:has(.qv-page) .modalfoot{flex:0 0 auto!important;margin-top:0!important;padding:10px 0!important}
      body:has(.qv-page) .modalbody .qv-page{margin-bottom:16px!important}
    `;
  ...
  function fitPage(page){
    ...
    const naturalW=page.offsetWidth||page.scrollWidth||1020;
    const naturalH=page.offsetHeight||page.scrollHeight||1400;
    const bodyW=m.body?.clientWidth||window.innerWidth;
    const availableW=Math.max(300,bodyW-48);
