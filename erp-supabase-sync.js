/* Kútfő Plusz ERP - Supabase sync for projects + work logs + layers + filters.
   Keeps the existing UI/local model, but makes Supabase the source of truth. */
(function(){
  'use strict';
  ...
  async function request(path,options){
    ...
    const headers=Object.assign({apikey:c.publishableKey,Authorization:'Bearer '+c.publishableKey,Accept:'application/json'},(options&&options.headers)||{});
  ...
  async function loadProjects(){
    await api().list().then(rows=>{
      const mapped=(rows||[]).map(r=>({
        id:r.project_number||r.id,
        supabaseId:r.id,
        customerId:r.customer_id||'',
        customerName:'',
        name:r.name||'', location:r.location||'', status:r.status||'Tervezés',
        value:num(r.contract_value), planned:num(r.planned_cost), cost:num(r.actual_cost), progress:0, notes:'',
        quoteId:r.quote_id||''
      }));
  ...
  async function loadWorklogs(){
    ...
    db.worklogs=(rows||[]).map(r=>{
      const d=r.document_data||{};
      const p=projectByRemoteId(r.project_id);
      return {id:localWorklogId(r),supabaseId:r.id,date:r.work_date||'',customerId:p?p.customerId:'',projectId:p?p.id:'',location:r.description?d.location:(d.location||''),wellNo:d.wellNo||'',finalDepth:num(r.depth_end),depth:num(r.depth_end),status:r.work_type||'Folyamatban',layers:(byLayer[r.id]||[]).map(x=>[String(x.depth_from??''),String(x.depth_to??''),x.material||'',x.note||'', '', '']),filters:(byFilter[r.id]||[]).map(x=>[String(x.depth_from??''),String(x.depth_to??''),x.filter_type||'Vak',String(x.length??''),x.note||'']),prodPipe:d.prodPipe||'',staticWL:d.staticWL??r.water_level??'',dynamicWL:d.dynamicWL??'',measureLiters:num(d.measureLiters),measureSeconds:num(d.measureSeconds),flow:num(d.flow),dynamic2:d.dynamic2||'',static2:d.static2||'',notes:r.notes||d.notes||'',_remoteProjectId:r.project_id||''};
    });
  }

  async function saveWorklogRemote(o){
    const p=projectByLocalId(o.projectId);
    const projectUuid=p?p.supabaseId:null;
    ...
    let row=null;
    if(o.supabaseId){
      const rows=await request('work_logs?id=eq.'+encodeURIComponent(o.supabaseId),{method:'PATCH',headers:{'Content-Type':'application/json','Prefer':'return=representation'},body:JSON.stringify(payload)}); row=rows&&rows[0];
    } else {
      const rows=await request('work_logs',{method:'POST',headers:{'Content-Type':'application/json','Prefer':'return=representation'},body:JSON.stringify(payload)}); row=rows&&rows[0];
    }
    if(!row) throw new Error('A munkanapló mentése nem adott vissza rekordot.');
    await request('well_layers?work_log_id=eq.'+encodeURIComponent(row.id),{method:'DELETE'});
    await request('work_log_filters?work_log_id=eq.'+encodeURIComponent(row.id),{method:'DELETE'});
  ...
  async function install(){
    ...
    if(typeof window.wlSave==='function'&&!window.__KP_WL_SUPABASE_HOOKED__){
      window.wlSave=async function(){
        try{
          document.querySelectorAll('#wl_layers .wl-layer-type').forEach((el,i)=>{if(window.wlLayers&&wlLayers[i])wlLayers[i][2]=el.value});
          const o=window.wlCollect();
          const remote=await saveWorklogRemote(o);
          o.supabaseId=remote.id;
          const idx=db.worklogs.findIndex(x=>String(x.id)===String(o.id));
          if(idx>=0)db.worklogs[idx]=o;else db.worklogs.push(o);
          if(typeof wlClearDraft==='function')wlClearDraft(o.id);
          if(typeof closeModal==='function')closeModal();
          if(typeof nav==='function')nav('worklogs');
          if(typeof toast==='function')toast('Munkanapló és rétegsor Supabase-ben mentve');
        }catch(e){console.error(e);if(typeof toast==='function')toast('Hiba: '+(e.message||e));}
      };
      window.__KP_WL_SUPABASE_HOOKED__=true;
    }

    if(typeof window.detailedWorklogEditor==='function'&&!window.__KP_WL_OPEN_HOOKED__){
      const original=window.detailedWorklogEditor;
      window.detailedWorklogEditor=function(id,projectId){
        return original(id,projectId);
      };
      window.__KP_WL_OPEN_HOOKED__=true;
    }
  }
  window.KPSupabaseSync={loadProjects,loadWorklogs,saveWorklogRemote};
  install();
})();
