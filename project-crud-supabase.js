...
(function(){
  ...
  async function update(id,project){if(!id)throw new Error('Hiányzó projektazonosító.');const payload=await clean(project);if(!payload.customer_id)throw new Error('A kiválasztott ügyfél nincs összekötve a Supabase customers táblával.');const rows=await request(TABLE+'?id=eq.'+encodeURIComponent(id),{method:'PATCH',headers:{'Content-Type':'application/json','Prefer':'return=representation'},body:JSON.stringify(payload)});const row=Array.isArray(rows)?rows[0]:rows;if(!row)throw new Error('A Supabase nem adott vissza módosított projektet az UUID alapján.');return row;}
  async function updateByProjectNumber(projectNumber,project){if(!projectNumber)throw new Error('Hiányzó projektazonosító.');const payload=await clean(project);if(!payload.customer_id)throw new Error('A kiválasztott ügyfél nincs összekötve a Supabase customers táblával.');const rows=await request(TABLE+'?project_number=eq.'+encodeURIComponent(projectNumber),{method:'PATCH',headers:{'Content-Type':'application/json','Prefer':'return=representation'},body:JSON.stringify(payload)});const row=Array.isArray(rows)?rows[0]:rows;if(!row)throw new Error('A Supabase nem adott vissza módosított projektet a projektazonosító alapján.');return row;}
  async function findByProjectNumber(projectNumber){const rows=await request(TABLE+'?select=*&project_number=eq.'+encodeURIComponent(projectNumber)+'&limit=1');return Array.isArray(rows)&&rows[0]?rows[0]:null;}
  async function remove(id){if(!id)throw new Error('Hiányzó projektazonosító.');const logs=await request('work_logs?select=id&project_id=eq.'+encodeURIComponent(id));if(Array.isArray(logs)&&logs.length)throw new Error('A projekt nem törölhető, mert munkanapló tartozik hozzá.');await request(TABLE+'?id=eq.'+encodeURIComponent(id),{method:'DELETE',headers:{'Prefer':'return=minimal'}});return true;}
